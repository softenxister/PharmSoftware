...
type StorePosSettingsRow = {
  ...
  billingDevice: unknown;
  paperSize: unknown;
  cashDrawerDevice: unknown;
...
export async function readStorePosSettings(): Promise<StorePosSettings> {
  const rows = await prisma.$queryRaw<StorePosSettingsRow[]>(Prisma.sql`
    SELECT "showProductLocation", "paymentMethods", "billingDevice", "paperSize", "cashDrawerDevice", "autoOpenCashDrawer"
...
export async function saveStorePosSettings(
  ...
  await prisma.$executeRaw(Prisma.sql`
    INSERT INTO "StorePosSettings" (
      "id", "showProductLocation", "paymentMethods", "billingDevice", "paperSize", "cashDrawerDevice", "autoOpenCashDrawer", "updatedBy", "createdAt", "updatedAt"
    ) VALUES (
      ${PRIMARY_STORE_SETTINGS_ID}, ${settings.showProductLocation}, ${paymentMethods}::jsonb, ${settings.billingDevice}, ${settings.paperSize}, ${settings.cashDrawerDevice}, ${settings.autoOpenCashDrawer}, ${updatedBy}, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
    )
    ON CONFLICT ("id") DO UPDATE SET
      "showProductLocation" = EXCLUDED."showProductLocation",
      "paymentMethods" = EXCLUDED."paymentMethods",
      "billingDevice" = EXCLUDED."billingDevice",
      "paperSize" = EXCLUDED."paperSize",
      "cashDrawerDevice" = EXCLUDED."cashDrawerDevice",
      "autoOpenCashDrawer" = EXCLUDED."autoOpenCashDrawer",
