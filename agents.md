...
- Stock inventory: `src/pages/stock/StockPage.tsx` is only the route seam; `src/features/stock/inventory/` owns authoritative inventory reads, filters, totals, pagination, and Product form composition. It composes the Product editor lifecycle rather than owning Product writes. Treat server results as authoritative and do not add a second client-side filtering pass.
