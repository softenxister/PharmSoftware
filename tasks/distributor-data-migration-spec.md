...
- Focused tests: `node --env-file-if-exists=.env --import tsx --test src/server/import/distributorDataMigration.test.ts src/server/import/distributorDataUpload.test.ts src/server/db/distributorDataMigrationRepository.test.ts server/apiRegistry.test.ts`
...
- `src/server/import/` validates files, extracts XLSX/CSV rows, and reconciles distributor identities.
- `src/server/db/` rechecks preview state and performs transactional creates/updates.
