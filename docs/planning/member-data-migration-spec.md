...
- Focused tests: `node --env-file-if-exists=.env --import tsx --test server/import/memberDataMigration.test.ts server/import/memberDataUpload.test.ts`
...
- `server/import/` parses UTF-8 CSV data, normalizes import values, classifies rows, and creates confirmation tokens.
- `server/db/` rechecks the preview and performs transactional member upserts.
