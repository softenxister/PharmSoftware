...
- TypeScript pure classification logic in `server/import/`.
...
- Targeted tests: `node --env-file-if-exists=.env --import tsx --test server/import/productCategoryNormalization.test.ts`
- Related tests: `node --env-file-if-exists=.env --import tsx --test server/import/productCategoryNormalization.test.ts server/import/cwStockNormalizer.test.ts server/import/cwStockMigration.test.ts src/features/stock/stockCategoryFilter.test.ts`
- Preview: `node --env-file-if-exists=.env --import tsx scripts/data-maintenance/normalize-product-categories.ts`
- Apply after review: `node --env-file-if-exists=.env --import tsx scripts/data-maintenance/normalize-product-categories.ts --apply`
- Type check: `./node_modules/.bin/tsc --noEmit --incremental false`
- Whitespace check: `git diff --check`

## Project Structure

- `server/import/productCategoryNormalization.ts` — pure classifier and evidence.
- `server/import/productCategoryNormalization.test.ts` — representative positive, conflict, and negative cases.
- `scripts/data-maintenance/normalize-product-categories.ts` — live preview, backup, and guarded apply.
- `src/lib/productCategories.ts` — stable English/Thai category definitions.
- `data/outputs/product-category-normalization-backups/` — recovery artifacts.
