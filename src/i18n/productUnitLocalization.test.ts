...
test("measurements are selectable only as subunits", () => {
  const subunitValues: readonly string[] = PRODUCT_SUBUNIT_VALUES;
  const unitValues: readonly string[] = PRODUCT_UNIT_VALUES;
  const packageValues: readonly string[] = PRODUCT_PACKAGE_VALUES;

  for (const measurement of ["kg", "g", "ml", "l"]) {
    assert.ok(subunitValues.includes(measurement), measurement);
    assert.ok(!unitValues.includes(measurement), measurement);
    assert.ok(!packageValues.includes(measurement), measurement);
  }
  for (const forbidden of ["mg", "mcg", "cc"]) {
    assert.ok(!subunitValues.includes(forbidden), forbidden);
    assert.ok(!unitValues.includes(forbidden), forbidden);
    assert.ok(!packageValues.includes(forbidden), forbidden);
  }
});
